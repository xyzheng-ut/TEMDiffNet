...
def load_dataset(path_train, path_val, batch_size=128):
    ...
    bulk_modulus = ds_train["bulk_modulus"].values
    mag_density = ds_train["mag_density"].values
    ...
    # plt.hist(band_gap, density=True, bins=30)
    # plt.show()
    ...
    # do mask
    mask = ~np.isnan(band_gap).ravel()
    x = x[mask]
    band_gap = band_gap[mask]

    x, y = sklearn.utils.shuffle(x, band_gap,random_state=222)
...
class PerformancePlotCallback(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        r2 = cal_r2(y, y_pred)
        # plot
        g = sns.jointplot(x=y * 8.,
                          y=y_pred * 8.,
                          xlim=[-0.2, 8.2], ylim=[-0.2, 8.2],
                          color=colors[0],
                          # s=8,
                          # alpha=0.4,
                          height=4)

        g.set_axis_labels("Reference band gap [eV]", "Predicted band gap [eV]")
        plt.tight_layout()
        # plt.title(f'Epoch: {epoch}, r2: {r2:.4f}', size=6)
        plt.savefig(f'training_results/mlp/band_gap_{epoch}_{r2:.4f}.svg')
        plt.close()
...
def main():
    ...
    performance_simple = PerformancePlotCallback(val_ds, mean_std)
    early_stop = keras.callbacks.EarlyStopping(monitor='loss', mode='min', patience=10)
    history = mlp.fit(train_ds, validation_data=val_ds, epochs=args.epochs,
    ...
    # Save to CSV
    hist_df.to_csv("training_history.csv", index=True)
    ...
    # ax2.plot(x, np.array(MSE[1:]).astype(np.float), "C2")

    ax.set_xlabel(r"Training epoch")
    ax.set_ylabel(r"Loss")
    fig.legend(["Training loss", "Validation loss"], ncol=1, loc="upper right")

    # ax.grid(True)
    fig.tight_layout()

    plt.savefig('training_results/mlp/loss.svg')
    plt.close()
    ...
    shap.plots.beeswarm(shap_values, show=False, max_display=15, color_bar_label="Value of band gap", plot_size=(6, 4))
