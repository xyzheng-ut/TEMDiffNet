...
def load_dataset(path_train, path_val, batch_size=128):
    ...
    # do mask
    mask = ~np.isnan(bulk_modulus).ravel()
    x = x[mask]
    bulk_modulus = bulk_modulus[mask]

    x, y = sklearn.utils.shuffle(x, bulk_modulus,random_state=222)
...
class PerformancePlotCallback(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        g.set_axis_labels("Reference bulk modulus [GPa]", "Predicted bulk modulus [GPa]")
        plt.tight_layout()
        # plt.title(f'Epoch: {epoch}, r2: {r2:.4f}', size=6)
        plt.savefig(f'training_results/mlp/bulk_modulus_{epoch}_{r2:.4f}.svg')
        plt.close()
...
def main():
    ...
    shap.plots.beeswarm(shap_values, show=False, max_display=15, color_bar_label="Value of bulk modulus", plot_size=(6, 4))
